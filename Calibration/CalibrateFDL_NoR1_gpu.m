...
% See also : CalibrateFDL_UVD_gpu

function [Px,Py,U,V,D,sigPx,sigPy,residN2,gradN2,gX,gY] = CalibrateFDL_NoR1_gpu(ViewsFFT,wx,wy,numDisp,lambda,PxInit,PyInit)
...
Reg = gpuArray(single(lambda*(L2'*L2)));%2nd order regularization term


%Initialization
if(exist('PxInit','var') && size(PxInit,1)==numInitViews && size(PxInit,2)==numDisp)
    Px = gpuArray(single(PxInit));
else
    Px = gpuArray(zeros(numInitViews,numDisp,'single'));
end
if(exist('PyInit','var') && size(PyInit,1)==numInitViews && size(PyInit,2)==numDisp)
    Py = gpuArray(single(PyInit));
else
    Py = gpuArray(zeros(numInitViews,numDisp,'single'));
end
...
sumPx=zeros(size(Px),'gpuArray'); sigPx=zeros(size(Px),'gpuArray');
sumPy=zeros(size(Py),'gpuArray'); sigPy=zeros(size(Py),'gpuArray');
