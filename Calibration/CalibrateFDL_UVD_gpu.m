...
function [Px,Py,U,V,D,sigPx,sigPy,sigU,sigV,sigD,residN2,gradN2] = CalibrateFDL_UVD_gpu(ViewsFFT,wx,wy,numDisp,lambda, uPosInit,vPosInit,DispsInit,varargin)
...
wx = single(wx(:,1:xC));
wx = gpuArray(permute(wx(:),[4 3 2 1]));
wy = single(wy(:,1:xC));
wy = gpuArray(permute(wy(:),[4 3 2 1]));
...
Reg = gpuArray(single(lambda*(L2'*L2)));
...
if(exist('uPosInit','var') && ~isempty(uPosInit))
    if( size(uPosInit,1)*size(uPosInit,2)==numInitViews && (size(uPosInit,3)==nChan || size(uPosInit,3)==1) )
        U = bsxfun(@times, reshape(gpuArray(single(uPosInit)),numInitViews,1,size(uPosInit,3)), ones(1,1,nChan) );
    else
...
elseif(fixed_UV)
    ...
    U = gpuArray(zeros(numInitViews,1,nChan,'single'));
end
if(exist('vPosInit','var') && ~isempty(uPosInit))
    if( size(vPosInit,1)*size(vPosInit,2)==numInitViews && (size(vPosInit,3)==nChan || size(vPosInit,3)==1) )
        V = bsxfun(@times, reshape(gpuArray(single(vPosInit)),numInitViews,1,size(vPosInit,3)), ones(1,1,nChan) );
    else
...
elseif(fixed_UV)
    ...
    V = gpuArray(zeros(numInitViews,1,nChan,'single'));
end
if(exist('DispsInit','var') && ~isempty(DispsInit))
    if( size(DispsInit,1)*size(DispsInit,2)==numDisp && (size(DispsInit,3)==nChan || size(DispsInit,3)==1) )
        D = bsxfun(@times, reshape(gpuArray(single(DispsInit)),1,numDisp,size(DispsInit,3)), ones(1,1,nChan) );
    else
...
elseif(fixed_D)
    ...
    D = repmat(gpuArray(linspace(0,10,numDisp)),1,1,nChan);
end
if(~fixed_D),  gradD = gpuArray(zeros(size(D),'single'));end
if(~fixed_UV), gradU = gpuArray(zeros(size(U),'single'));
               gradV = gpuArray(zeros(size(V),'single'));
end
gradN2=single(gpuArray([]));
...
ViewsFFT_Eval_gpu = reshape(gpuArray(ViewsFFT(:,:,freqEvalIds)),numInitViews,1,nChan,[]);

%Mean and standard deviation of the parameters at iterations after reaching the stopping criterion.
sumPx=zeros(length(U),length(D),nChan,'gpuArray'); sigPx=zeros(length(U),length(D),nChan,'gpuArray');
sumPy=zeros(length(U),length(D),nChan,'gpuArray'); sigPy=zeros(length(U),length(D),nChan,'gpuArray');
sumU=zeros(size(U),'gpuArray');   sigU=zeros(size(U),'gpuArray');
sumV=zeros(size(V),'gpuArray');   sigV=zeros(size(V),'gpuArray');
sumD=zeros(size(D),'gpuArray');   sigD=zeros(size(D),'gpuArray');
...
while(iter <= numIterMax)
    ...
    ViewsFFT_gpu = reshape(gpuArray(ViewsFFT(:,:,freqIds)),numInitViews,1,nChan,[]);
...
%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Z = pagefun(@mldivide, ...
            bsxfun(@plus, pagefun(@mtimes,AT,A), Reg),...
            pagefun(@mtimes,AT,ViewsFFT_gpu));
    
    resid = pagefun(@mtimes,A,Z)-ViewsFFT_gpu;
...
if(mod(iter,EvalPeriod)==0 && ~EvaluatingSigError)
    ...
    Z = pagefun(@mldivide, ...
            bsxfun(@plus, pagefun(@mtimes,AT,A), Reg),...
            pagefun(@mtimes,AT,ViewsFFT_Eval_gpu));
    resid = pagefun(@mtimes,A,Z)-ViewsFFT_Eval_gpu;
