...
function [Px,Py,U,V,D,sigPx,sigPy,sigU,sigV,sigD,residN2,gradN2] = CalibrateFDL_UVD_gpu(ViewsFFT,wx,wy,numDisp,lambda, uPosInit,vPosInit,DispsInit)
...
wx = single(wx(:,1:xC));
wx = gpuArray(permute(wx(:),[3 2 1]));
wy = single(wy(:,1:xC));
wy = gpuArray(permute(wy(:),[3 2 1]));
...
%Initialization
gradPx = zeros(numInitViews,numDisp,'gpuArray');
gradPy = zeros(numInitViews,numDisp,'gpuArray');

rng('default');rng(0);

if(exist('uPosInit','var') && numel(uPosInit)==numInitViews)
    U = gpuArray(single(uPosInit(:)));
else
    U = gpuArray(zeros(numInitViews,1,'single'));
end
if(exist('vPosInit','var') && numel(vPosInit)==numInitViews)
    V = gpuArray(single(vPosInit(:)));
else
    V = gpuArray(zeros(numInitViews,1,'single'));
end
if(exist('DispsInit','var') && numel(DispsInit)==numDisp)
    D = gpuArray(single(DispsInit(:)'));
else
    D = gpuArray(linspace(0,10,numDisp));
end
gradD = gpuArray(zeros(size(D),'single'));
gradU = gpuArray(zeros(size(U),'single'));
...
ViewsFFT_Eval_gpu = gpuArray(ViewsFFT(:,:,freqEvalIds));
rng('default');rng(0);
...
sumPx=zeros(length(U),length(D),'gpuArray'); sigPx=zeros(length(U),length(D),'gpuArray');
sumPy=zeros(length(U),length(D),'gpuArray'); sigPy=zeros(length(U),length(D),'gpuArray');
...
while(iter <= numIterMax)
    ...
    ViewsFFT_gpu = gpuArray(ViewsFFT(:,:,freqIds));
